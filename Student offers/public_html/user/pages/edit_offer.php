...
if(is_post_request()) {
  ...
  $result = update_offer($offer);  /*updating offer in DB*/

    if($result === true) {
        $_SESSION['message'] = 'The offer was updated successfully.'; /* Store Success msg in sessions */
        redirect_to(url_for('/user/pages/show_user_offers.php?id=' . $id));
    } else {
        $errors = $result;
    }
...
<form action="<?php echo url_for('/user/pages/edit_offer.php?id=' . h(u($id))); ?>" method="post">
  <div class="" style="overflow-x:auto;">
    <table class="post-form" style="background-color:#e4e5e6">
      ...
      <tr>
        ...
        <td>
          <input style="width:95%" id="headline" name="headline" placeholder="headline" required="" tabindex="1" type="text" value="<?php echo h($offer['headline']); ?>">
        </td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td><input style="width:95%" id="price" name="price" placeholder="price" required="" tabindex="3" type="text" value="<?php echo h($offer['price']); ?>"></td>
      </tr>
      <tr>
        <td><label for="street">Street and Building</label></td>
        <td>	<input style="width:60%" id="street" name="street" placeholder="Street" required="" tabindex="4" type="text" value="<?php echo h($offer['street']); ?>">
          <input style="width:35%;display:inline"id="building" name="building" placeholder="Building" required="" tabindex="5" type="text" value="<?php echo h($offer['building']); ?>">
        </td>
      </tr>
      <tr>
        <td>  <label for="zip">Zip code / City</label></td>
        <td>	<input style="width:40%" id="zip" name="zip" placeholder="Zip Code" required="" tabindex="6" type="text" value="<?php echo h($offer['zip']); ?>">
...
<a class="button" style="display:inline;float:right;width:90px;margin-top:5px;margin-right:5px;font-size:14px" href="<?php echo url_for('/user/pages/show_user_offers.php?id=' . $id);?>" >Cancel</a>
</li>
</ul>
</section><!-- .buckets -->
